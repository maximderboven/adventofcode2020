    ...
    public static void main(String[] args) {
        ...
        for (String il : inputlist) {
            if ((il.contains("byr:") && il.contains("iyr:") && il.contains("eyr:") && il.contains("hgt:") && il.contains("hcl:") && (il.contains("ecl:") && il.contains("pid:") && il.contains("cid:")) || il.contains("byr:") && il.contains("iyr:") && il.contains("eyr:") && il.contains("hgt:") && il.contains("hcl:") && il.contains("ecl:") && il.contains("pid:"))) {
                ...
                String ecl = "";
                String pid = "";
                String[] aeye = {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"};


                cutted = il.split("\\s");
                //"iyr:2010";
                for (String cl : cutted) {

                    if (cl.contains("byr")) {
                        byr = Integer.parseInt(cl.split(":")[1]);
                    }

                    if (cl.contains("iyr")) {
                        iyr = Integer.parseInt(cl.split(":")[1]);
                    }

                    if (cl.contains("eyr")) {
                        eyr = Integer.parseInt(cl.split(":")[1]);
                    }
                ...
                if (String.valueOf(byr).length() == 4 && byr >= 1920 && byr <= 2020) {
                    if (String.valueOf(iyr).length() == 4 && iyr >= 2010 && iyr <= 2020) {
                        if (String.valueOf(eyr).length() == 4 && eyr >= 2020 && eyr <= 2030) {
                            if ((hgt.contains("cm") && (Integer.parseInt(hgt.substring(0, hgt.length() - 2)) >= 150) && (Integer.parseInt(hgt.substring(0, hgt.length() - 2)) <= 193)) || (hgt.contains("in") && (Integer.parseInt(hgt.substring(0, hgt.length() - 2)) >= 59) && (Integer.parseInt(hgt.substring(0, hgt.length() - 2)) <= 76))) {
                                if (hcl.matches("[#][0-9a-f]{6}")) {
                                    //hcl.charAt(0) == '#' && hcl.length() == 7 &&
                                    if (Arrays.asList(aeye).contains(ecl)) {
